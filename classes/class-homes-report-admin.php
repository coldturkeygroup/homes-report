...
class HomesReport_Admin
{
    ...
    /**
     * Basic constructor for the Homes Report Admin class
     *
     * @param string $file
     */
    public function __construct($file)
    {
        ...
        // Register homes report settings
    ...
    public function add_menu_item()
    {
        ...
        add_submenu_page('edit.php?post_type=' . $this->token, 'Homes Report Settings', 'Settings', 'manage_options', $this->token . '_settings', [
            $this,
            'settings_page'
        ]);
    }
    ...
    public function admin_notices()
    {
        ...
        if ($wp_version < 3.5) {
            ?>
            <div class="error">
                <p><?php printf(__('%1$sHomes Report%2$s requires WordPress 3.5 or above in order to function correctly. You are running v%3$s - please update now.', $this->token), '<strong>', '</strong>', $wp_version); ?></p>
            </div>
            <?php
        }
    }
    ...
    public function register_settings()
    {
        ...
        // Add settings fields
        add_settings_field($this->token . '_slug', __('URL slug for homes report pages:', $this->token), [
            $this,
            'slug_field'
    ...
    {
        echo '<p>' . __('These are a few simple settings for setting up your homes report pages.', $this->token) . '</p>';
    }

    /**
     * Create the slug field for the Settings page.
     * The slug field allows users to choose which
     * subdirectory their homes report pages are nested in.
     *
     */
    public function slug_field()
    {
        ...
        echo '<input id="slug" type="text" name="' . $this->token . '_slug" value="' . $data . '"/>
				<label for="slug"><span class="description">' . sprintf(__('Provide a custom URL slug for the homes report pages.', $this->token)) . '</span></label>';
    }
    ...
    public function settings_page()
    {
        ...
        echo '<div class="wrap" id="' . $this->token . '_settings">
					<h1>' . __('Homes Report Settings', $this->token) . '</h1>
