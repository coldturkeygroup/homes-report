...
class HomesReport
{
    ...
    /**
     * Basic constructor for the Homes Report class
     *
     * @param string $file
    ...
    public function __construct($file, FrontDesk $frontdesk)
    {
        ...
        // Register 'pf_homes_report' post type
    ...
    /**
     * Registers the Homes Report custom post type
    ...
    {
        $labels = [
            'name' => _x('Homes Reports', 'post type general name', $this->token),
            'singular_name' => _x('Homes Report', 'post type singular name', $this->token),
            'add_new' => _x('Add New', $this->token, $this->token),
            'add_new_item' => sprintf(__('Add New %s', $this->token), __('Homes Report', $this->token)),
            'edit_item' => sprintf(__('Edit %s', $this->token), __('Homes Report', $this->token)),
            'new_item' => sprintf(__('New %s', $this->token), __('Homes Report', $this->token)),
            'all_items' => sprintf(__('All %s', $this->token), __('Homes Reports', $this->token)),
            'view_item' => sprintf(__('View %s', $this->token), __('Homes Report', $this->token)),
            'search_items' => sprintf(__('Search %a', $this->token), __('Homes Reports', $this->token)),
            'not_found' => sprintf(__('No %s Found', $this->token), __('Homes Reports', $this->token)),
            'not_found_in_trash' => sprintf(__('No %s Found In Trash', $this->token), __('Homes Reports', $this->token)),
            'parent_item_colon' => '',
            'menu_name' => __('Homes Reports', $this->token)
        ];

        $slug = __('homes-report', $this->token);
    ...
    public function updated_messages($messages)
    {
        ...
        $messages[$this->token] = [
            ...
            4 => __('Homes Report updated.', $this->token),
            /* translators: %s: date and time of the revision */
            5 => isset($_GET['revision']) ? sprintf(__('Homes Report restored to revision from %s.', $this->token), wp_post_revision_title((int)$_GET['revision'], false)) : false,
            6 => sprintf(__('Homes Report published. %sView Homes Report%s.', $this->token), '<a href="' . esc_url(get_permalink($post_ID)) . '">', '</a>'),
            7 => __('Homes Report saved.', $this->token),
            8 => sprintf(__('Homes Report submitted. %sPreview Homes Report%s.', $this->token), '<a target="_blank" href="' . esc_url(add_query_arg('preview', 'true', get_permalink($post_ID))) . '">', '</a>'),
            9 => sprintf(__('Homes Report scheduled for: %1$s. %2$sPreview Homes Report%3$s.', $this->token), '<strong>' . date_i18n(__('M j, Y @ G:i', $this->token), strtotime($post->post_date)) . '</strong>', '<a target="_blank" href="' . esc_url(get_permalink($post_ID)) . '">', '</a>'),
            10 => sprintf(__('Homes Report draft updated. %sPreview Homes Report%s.', $this->token), '<a target="_blank" href="' . esc_url(add_query_arg('preview', 'true', get_permalink($post_ID))) . '">', '</a>'),
        ];
     ...
     * for our Homes Report post type creator & editor.
     *
     */
     ...
     * in the meta box for our Homes Report post type.
     *
     * @param $post
     * @param $meta
     */
     ...
     * the custom fields for our Homes Report post type.
     *
    ...
    public function enqueue_scripts()
    {
        if (is_singular($this->token)) {
            wp_register_style($this->token, esc_url($this->assets_url . 'css/homesreport.css'), [], HOMES_REPORT_PLUGIN_VERSION);
    ...
    public function get_custom_fields_settings($meta_box)
    {
        ...
        if ($meta_box == 'basic' || $meta_box == 'all') {
            ...
            $fields['primary_color'] = [
                ...
                'description' => __('Change the primary color of the homes report page.', $this->token),
                'placeholder' => '',
                'type' => 'color',
                'default' => '',
                'section' => 'info'
            ...
            $fields['hover_color'] = [
                ...
                'description' => __('Change the button hover color of the homes report page.', $this->token),
                'placeholder' => '',
                'type' => 'color',
                'default' => '',
                'section' => 'info'
     ...
     * for a defined Homes Report created page.
     *
     * @param integer $post_ID
     *
     * @return bool
    ...
    protected function emailResultsToAdmin($user_id, $page_id)
    {
        ...
        $subject = 'New Homes Report Submission';
    ...
    public function change_default_title($title)
    {
        ...
        if ($this->token == $screen->post_type) {
            $title = 'Enter a title for your homes report page';
        }
